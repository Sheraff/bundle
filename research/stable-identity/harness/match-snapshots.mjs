...
function weightedIntersection(fromWeights, toWeights) {
  let total = 0;
...
function weightedCoverage(sourceWeights, targetWeightsList) {
  let total = 0;
  let covered = 0;
...
function getPrimaryIdentity(chunk) {
  return chunk.manifestSourceKeys[0] ?? chunk.facadeModule?.stableId ?? null;
}

function matchRootChunks(fromChunks, toChunks) {
  const same = [];
  const added = [];
  const removed = [];
...
function scoreSharedChunkCandidate(fromChunk, toChunk) {
  ...
  const score = moduleScore * 0.8 + ownerScore * 0.2;
...
function scoreSharedChunkFallback(fromChunk, toChunk) {
  const ownerScore = jaccard(fromChunk.ownerRoots, toChunk.ownerRoots);
  ...
  const similarModuleCount =
    sizeRatio(fromChunk.moduleIds.length, toChunk.moduleIds.length) >= 0.5;
...
function matchSharedChunks(fromChunks, toChunks) {
  ...
  const matchedTo = new Set();
  const ambiguousFrom = new Set();
  const ambiguousTo = new Set();
  ...
  for (const fromChunk of fromChunks) {
    ...
    const reverseCandidates = fromChunks
      ...
      .map((chunk) => ({ chunk, score: scoreSharedChunkFallback(chunk, candidates[0].chunk) }))
      .filter((entry) => entry.score)
      .sort((left, right) => right.score.score - left.score.score);
    ...
    matchedFrom.add(fromChunk.fileName);
    matchedTo.add(candidates[0].chunk.fileName);
    same.push({
      relation: 'same',
      from: fromChunk.fileName,
      to: candidates[0].chunk.fileName,
      confidence: 'low',
      evidence: candidates[0].score.evidence,
    });
  }
...
function exactAssetKeyMatches(fromAssets, toAssets) {
  ...
  for (const asset of fromAssets) {
    ...
    if (exactCandidates.length === 1) {
      ...
      same.push({
        ...
        confidence: 'exact',
        evidence: [
          `source:${asset.sourceKeys.find((key) => target.sourceKeys.includes(key))}`,
        ],
      });
    }
  }
...
function addLineage(map, key, values) {
  const currentValues = map.get(key) ?? [];
  map.set(key, sortUnique([...currentValues, ...values]));
}

function buildChunkLineage(entries, dynamicEntries, sharedChunks) {
...
function getLineageTargets(importerFiles, lineageMap) {
  const targets = [];
...
function lineageCoverage(expectedFiles, actualFiles) {
...
function scoreAssetFallback(fromAsset, toAsset, chunkLineage) {
...
function matchAssets(fromAssets, toAssets, chunkLineage) {
  ...
  for (const fromAsset of fromAssets) {
    ...
    const candidates = toAssets
      ...
      .map((asset) => ({
        asset,
        score: scoreAssetFallback(fromAsset, asset, chunkLineage),
      }))
      .filter((entry) => entry.score)
  ...
  for (const toAsset of toAssets) {
    ...
    const candidates = fromAssets
      ...
      .map((asset) => ({
        asset,
        score: scoreAssetFallback(asset, toAsset, chunkLineage),
      }))
      .filter((entry) => entry.score)
  ...
  for (const fromAsset of fromAssets) {
    ...
    if (candidates.length === 1 && !multiLineage) {
      matchedFrom.add(fromAsset.fileName);
  ...
  const removed = fromAssets
    ...
    .map((asset) => ({ relation: 'removed', from: asset.fileName }));
  const added = toAssets
    .filter((asset) => !matchedTo.has(asset.fileName))
    .map((asset) => ({ relation: 'added', to: asset.fileName }));

  return { same, split, merge, removed, added };
}

function summarizeModules(fromAnalysis, toAnalysis) {
  const fromModules = new Map();
  const toModules = new Map();
...
function summarizeSharedChunks(sharedChunks) {
  const matchedFrom = new Set([
...
export function matchSnapshotPair(fromSnapshot, toSnapshot) {
  ...
  return {
    from: {
      fixtureId: fromSnapshot.fixtureId,
      versionId: fromSnapshot.versionId,
    },
    to: {
      fixtureId: toSnapshot.fixtureId,
      versionId: toSnapshot.versionId,
    },
    entries,
    dynamicEntries,
    sharedChunks,
