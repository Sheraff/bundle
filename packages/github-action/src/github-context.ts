...
export async function collectGithubContext(
  ...
  const repositoryPartsResult = v.safeParse(repositoryNamePartsSchema, splitRepositoryName(repositoryName))
...
async function readGithubEventPayload(eventPath: string): Promise<GithubEventPayload> {
  ...
  try {
    ...
    throw new Error(`Could not read the GitHub event payload at ${eventPath}`, { cause: error })
  }
...
function normalizeRequiredInput(value?: string) {
  return value?.trim() ?? ''
}

function normalizeTrimmedInput(value?: string) {
...
function resolvePullRequestContext(eventPayload: GithubEventPayload) {
  ...
  if (!result.success) {
    throw new Error(`Invalid pull request context in GitHub event payload: ${formatIssues(result.issues)}`)
  }
