...
export async function collectGithubContext(
	...
	const environmentResult = v.safeParse(
	...
	if (!environmentResult.success) {
		throw new Error(
			`Invalid GitHub runtime environment: ${formatIssues(environmentResult.issues)}`,
		);
	}

	const githubEnvironment = environmentResult.output;
	const eventPayload = await readGithubEventPayload(
		githubEnvironment.GITHUB_EVENT_PATH,
	);
	...
	if (!githubRepoId) {
		throw new Error(
			"Could not determine GITHUB_REPOSITORY_ID for the current run",
		);
	}
	...
	if (!repositoryPartsResult.success) {
		throw new Error(
			`Invalid GitHub repository name: ${formatIssues(repositoryPartsResult.issues)}`,
		);
	}

	const branch = resolveBranchName(
		eventPayload,
		githubEnvironment.GITHUB_REF_NAME,
	);
	...
	return {
		ci: {
			...
			workflowRunAttempt:
				parsePositiveInteger(githubEnvironment.GITHUB_RUN_ATTEMPT) ?? undefined,
...
async function readGithubEventPayload(
	...
	try {
		...
		throw new Error(
			`Could not parse the GitHub event payload at ${eventPath}`,
			{ cause: error },
		);
	}
	...
	if (
		...
		throw new Error(
			`The GitHub event payload at ${eventPath} is not a JSON object`,
		);
	}
...
function normalizeRepositoryNameFromPayload(
