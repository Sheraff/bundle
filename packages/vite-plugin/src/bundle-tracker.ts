...
export function bundleTracker(options: BundleTrackerOptions): Plugin {
  ...
  const artifactFile = path.resolve(process.cwd(), DEFAULT_ARTIFACT_RELATIVE_PATH)
  ...
  return {
    ...
    generateBundle(_, bundle) {
      ...
      if (capturedEnvironments.has(environmentName)) {
        throw new Error(`Environment name "${environmentName}" was emitted more than once in one build`)
      }
      ...
      for (const fileName of Object.keys(bundle).sort((left, right) => left.localeCompare(right))) {
        ...
        if (output.type === 'chunk') {
          chunks.push(
            serializeChunk(output as unknown as ChunkWithViteMetadata, environmentName, (rawId) =>
              getModuleOriginalLength(this, rawId),
            ),
          )
          continue
        }
        ...
        assets.push(serializeAsset(output as AssetWithBundleMetadata, environmentName))
      }
...
function serializeChunk(
  ...
  const modules = Object.entries(chunk.modules).map(([rawId, moduleInfo]) => ({
    rawId: requireNonEmptyString(rawId, `Environment "${environmentName}" has a chunk module with an empty id`),
...
async function readManifest(filePath: string, environmentName: string): Promise<CandidateManifest> {
  let contents: string
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error(`Environment "${environmentName}" has an invalid manifest payload at ${filePath}`)
  }
...
function normalizeResolvedManifestValue(value: unknown, environmentName: string) {
  if (value === true || typeof value === 'string') {
    return value
  }

  throw new Error(`Environment "${environmentName}" is missing build.manifest after config resolution`)
}
...
function sortUniqueStrings(values: Iterable<string>) {
  return [...new Set([...values].map((value) => requireNonEmptyString(value, 'Expected a non-empty string')))].sort(
