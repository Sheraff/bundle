...
export const SCENARIO_SOURCE_KINDS = ["fixture-app", "repo-synthetic", "hosted-synthetic"] as const
export const CI_PROVIDERS = ["github-actions"] as const
export const SCENARIO_RUN_STATUSES = ["queued", "processing", "processed", "failed"] as const
...
export const ulidSchema = v.pipe(nonEmptyStringSchema, v.regex(ULID_REGEX, "Expected a ULID"))
...
export const nonEmptyStringArraySchema = v.pipe(v.array(nonEmptyStringSchema), v.nonEmpty())

export const stringRecordSchema = v.record(nonEmptyStringSchema, nonEmptyStringSchema)

export const nonAllStringSchema = v.pipe(
  nonEmptyStringSchema,
  v.check((value) => value !== "all", '"all" is only valid on history and scenario pages'),
)
